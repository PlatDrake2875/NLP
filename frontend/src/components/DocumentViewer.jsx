...
export function DocumentViewer({ onBackToChat }) {
  ...
  const availableSources = useMemo(() => {
    ...
    allDocuments.forEach(doc => {
      sources.add(doc.metadata?.original_filename || doc.metadata?.source || 'Unknown Source');
    });
  ...
  const filteredDocuments = useMemo(() => {
    ...
    return allDocuments.filter(doc => {
      const sourceName = doc.metadata?.original_filename || doc.metadata?.source || 'Unknown Source';
  ...
  return (
    <div className={styles.documentViewer}>
        ...
        <div className={styles.headerControls}>
            ...
            {availableSources.length > 0 && !isLoading && (
                <div className={styles.sourceSelectorContainer}>
                    <label htmlFor="source-select" className={styles.sourceSelectLabel}>Select Document:</label>
                    <select
                        id="source-select"
      ...
      <div className={styles.viewerContent}>
        ...
        {!isLoading && !error && !selectedSource && availableSources.length > 0 && (
          <p className={styles.noDocumentsMessage}>Please select a document from the dropdown above.</p>
        )}
