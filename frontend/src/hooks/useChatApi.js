...
export function useChatApi(apiBaseUrl, activeSessionId, setSessions) {
  ...
  const handleChatSubmit = useCallback(async (query, model) => {
    ...
    console.log(`API Hook: [${activeSessionId} - ${reqId}] SUBMIT starting for model: ${model}`);
  ...
  const handleAutomateConversation = useCallback(async (jsonInputString, model, automationTask = null) => {
    if (!activeSessionId || !model || isSubmitting) {
      setAutomationError("Automation cannot start: Another process is running, or no session/model selected.");
      console.warn("API Hook: Automate submission prevented.", { activeSessionId, model, isSubmitting });
      return;
    }
    ...
    console.log(`API Hook: [${activeSessionId}] AUTOMATE starting with model: ${model}, task: ${automationTask || 'N/A'}`);
    console.log(`API Hook: [${activeSessionId}] Sending conversation history:`, formattedConversationHistory);
    setIsSubmitting(true);
    setAutomationError(null);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try { 
            ...
            try { const textError = await response.text(); errorDetail = `${errorDetail}: ${textError.substring(0,200)}`; } catch (e2) {}
        }
        throw new Error(errorDetail);
      }

      const result = await response.json(); // Expects AutomateResponse: {status, message, data}
      console.log(`API Hook: [${activeSessionId}] AUTOMATE success response:`, result);
      ...
      if (result.status === 'success' && result.data) {
        ...
        if (Object.keys(result.data).length === 0 && result.message) {
            ...
            automationMessageContent = "Automation completed, but no specific data returned.";
        }
        ...
        throw new Error(result.message || result.error_details || "Automation processing failed on backend.");
      }
      ...
      setSessions(prevSessions => {
        if (!prevSessions[activeSessionId]) return prevSessions;
        const currentHistory = prevSessions[activeSessionId].history || [];
        const errorMsg = { id: `error-automation-${Date.now()}`, sender: 'system', text: `⚠️ ${errorMessage}`, isLoading: false };
