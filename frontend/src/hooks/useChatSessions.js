...
export function useChatSessions(apiBaseUrl) {
  ...
  const handleSelectSession = useCallback((sessionId) => {
    ...
    if (sessions && sessions[sessionId]) {
        ...
        console.warn(`useChatSessions: Attempted to select non-existent session ${sessionId}`);
  ...
  const handleDeleteSession = useCallback((sessionIdToDelete) => {
      ...
      const nextActiveId = deleteSessionInternal(sessionIdToDelete, activeSessionId);
  ...
  }, [sessions, deleteSessionInternal, activeSessionId, setActiveSessionId, setAutomationError]);
  ...
  }, [activeSessionId, sessions, clearSessionHistoryInternal, setAutomationError]);
