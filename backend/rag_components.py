...
try:
    ...
    logger.warning(
        "OLLAMA_MODEL_FOR_AUTOMATION not found in config.py, defaulting to OLLAMA_MODEL_FOR_RAG or 'llama3'"
    )
    OLLAMA_MODEL_FOR_AUTOMATION = (
        OLLAMA_MODEL_FOR_RAG if "OLLAMA_MODEL_FOR_RAG" in globals() else "llama3"
    )
...
def format_history_for_lc(history: list[dict[str, str]]) -> list:
    ...
    for msg in history:
        ...
        if role.lower() == "user":
            ...
            logger.warning(
                f"Unknown role '{role}' in history, treating as HumanMessage."
            )
            lc_messages.append(HumanMessage(content=content))
    return lc_messages
...
def setup_rag_components():
    ...
    try:
        ...
        logger.info(
            f"HuggingFace embeddings initialized with model: {EMBEDDING_MODEL_NAME}"
        )
    ...
    if embedding_function:
        try:
            ...
            logger.info(
                f"LangChain Chroma vector store initialized for collection: {COLLECTION_NAME}"
            )
        ...
        logger.warning(
            "Skipping ChromaDB/Vectorstore/Retriever setup because embedding function failed."
        )
        chroma_client = vectorstore = retriever = None
    ...
    if OLLAMA_BASE_URL:
        try:
            logger.info(
                f"Initializing ChatOllama (for RAG) with model: {OLLAMA_MODEL_FOR_RAG} at {OLLAMA_BASE_URL}"
            )
        ...
        logger.warning(
            "OLLAMA_BASE_URL not set. ChatOllama for RAG cannot be initialized."
        )
        ollama_chat_for_rag = None
    ...
    if OLLAMA_BASE_URL:
        try:
            logger.info(
                f"Initializing ChatOllama (for Automation) with model: {OLLAMA_MODEL_FOR_AUTOMATION} at {OLLAMA_BASE_URL}"
            )
            ...
            logger.info(
                f"ChatOllama (for Automation) initialized with model {OLLAMA_MODEL_FOR_AUTOMATION}."
            )
        except Exception as e:
            logger.error(
        ...
        logger.warning(
            "OLLAMA_BASE_URL not set. ChatOllama for Automation cannot be initialized."
        )
        ollama_chat_for_automation = None
...
def get_chroma_client() -> chromadb.HttpClient:
    if chroma_client is None:
        logger.error(
            "Global ChromaDB client is None. Setup might have failed or is incomplete."
        )
...
def get_vectorstore() -> LangchainChroma:
    if vectorstore is None:
        logger.error(
            "Global Vectorstore is None. Setup might have failed or is incomplete."
        )
...
def get_retriever() -> Any:
    if retriever is None:
        logger.error(
            "Global Retriever is None. Setup might have failed or is incomplete."
        )
...
def get_ollama_chat_for_rag() -> ChatOllama:
    if ollama_chat_for_rag is None:
        logger.error(
            "Global ChatOllama (for RAG) is None. Setup failed or OLLAMA_BASE_URL is not set."
        )
...
def get_llm_for_automation() -> ChatOllama:
    ...
    if ollama_chat_for_automation is None:
        logger.error(
            "Global ChatOllama (for Automation) is None. Setup failed, OLLAMA_BASE_URL is not set, or OLLAMA_MODEL_FOR_AUTOMATION is problematic."
        )
...
async def get_rag_context_prefix(query: str) -> Optional[str]:
    ...
    if not retriever or not rag_context_prefix_prompt_template_obj:
        logger.warning(
            "Retriever or RAG context prefix prompt template not initialized. Cannot generate prefix."
        )
        return None

    try:
        logger.info(
            f"Invoking retriever for RAG context prefix with query: '{query[:50]}...'"
        )
        retrieved_docs = await retriever.ainvoke(query)

        if not retrieved_docs:
            logger.info(
                "No relevant documents found by retriever for RAG context prefix."
            )
            return None
        ...
        if rag_context_prefix_prompt_template_obj:
            ...
            logger.info(
                f"Generated RAG context prefix string (length: {len(formatted_prompt)})."
            )
            return formatted_prompt
        else:
            logger.error(
                "rag_context_prefix_prompt_template_obj is None. Cannot format prompt."
            )
            return None
