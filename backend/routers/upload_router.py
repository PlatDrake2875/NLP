...
from fastapi import (  # Import Depends
    APIRouter,
    Depends,
    File,
    HTTPException,
...
async def upload_document_endpoint(
    ...
    logger.info(
        f"Upload request received for: {original_filename}, Content-Type: {file.content_type}"
    )
    ...
    try:
        ...
        try:
            ...
            logger.error(
                f"Error reading uploaded file '{original_filename}': {read_err}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=400, detail=f"Could not read uploaded file: {str(read_err)}"
            ) from read_err
        ...
        try:
            ...
            logger.info(
                f"File '{original_filename}' saved temporarily to {temp_file_path}"
            )
        except OSError as save_err:
            logger.error(
                f"Error saving temporary file '{temp_file_path}': {save_err}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
                detail=f"Could not save uploaded file temporarily: {str(save_err)}",
            ) from save_err
        ...
        if file.content_type == "application/pdf":
            try:
                ...
                logger.error(
                    f"Error loading PDF '{original_filename}' with PyPDFLoader: {pdf_err}",
                    exc_info=True,
                )
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Failed to load PDF content. File might be corrupted or invalid: {str(pdf_err)}",
                ) from pdf_err
        else:
            if os.path.exists(temp_file_path):
                os.remove(temp_file_path)
        ...
        try:
            ...
            logger.info(
                f"Split content from '{original_filename}' into {len(chunks)} chunks."
            )
        except Exception as split_err:
            logger.error(
                f"Error splitting document '{original_filename}': {split_err}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
                detail=f"Failed to split document content: {str(split_err)}",
            ) from split_err

        if not chunks:
            logger.warning(
                f"No text chunks were generated from file: {original_filename}"
            )
            raise HTTPException(
                status_code=400,
        ...
        try:
            ...
            logger.info(
                f"Successfully added {len(chunks)} chunks from '{original_filename}' to the vector store."
            )
        except Exception as vs_err:
            logger.error(
                f"Error adding document chunks from '{original_filename}' to vector store: {vs_err}",
                exc_info=True,
            )
            raise HTTPException(
                status_code=500,
                detail=f"Failed to add document to vector store: {str(vs_err)}",
            ) from vs_err
        ...
        logger.error(
            f"HTTPException during upload of '{original_filename}': Status={e_http.status_code}, Detail={e_http.detail}"
        )
        raise e_http
    except Exception as e:
        logger.error(
            f"Unexpected error processing upload for file '{original_filename}': {e}",
            exc_info=True,
        )
        raise HTTPException(
            status_code=500,
            detail=f"An unexpected server error occurred while processing the file: {str(e)}",
        ) from e
    finally:
        ...
        if os.path.exists(temp_file_path):
            try:
                ...
                logger.error(
                    f"Error removing temporary file '{temp_file_path}': {e_os}"
                )
        await file.close()
