...
async def health_check_endpoint(
    ...
    if ollama_chat_for_rag and OLLAMA_BASE_URL:  # Check if the injected instance exists
        try:
            ...
            ollama_status = HealthStatusDetail(
                status="disconnected", details=error_detail
            )
            logger.warning(f"Ollama health check: FAILED - {error_detail}")
    else:
        logger.warning(
            f"Ollama health check: SKIPPED - ollama_chat_for_rag is {'available' if ollama_chat_for_rag else 'unavailable'}, OLLAMA_BASE_URL is {OLLAMA_BASE_URL}"
        )

    # --- Check ChromaDB Status ---
    chroma_status = HealthStatusDetail(
        status="disconnected",
    ...
    if chroma_client:  # Check if the injected instance exists
        ...
        logger.warning(
            "ChromaDB health check: SKIPPED - chroma_client is not initialized."
        )

    # --- Determine Overall Status ---
    overall_status = (
        "ok"
        if ollama_status.status == "connected" and chroma_status.status == "connected"
        else "error"
    )
    ...
    if overall_status == "error":
        logger.warning(
            f"Health check returning status: ERROR. Details: {response_payload}"
        )
        detail_payload = (
