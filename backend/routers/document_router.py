    ...
    try:
        ...
        for attr_name in possible_client_attrs:
            if hasattr(vectorstore, attr_name):
                ...
                if potential_client is not None and hasattr(potential_client, 'get_collection'):
                    client = potential_client
                    client_type_found = str(type(client))
                    logger.info(f"Using object from attribute '{attr_name}' as the client.")
        ...
        if client is None:
             available_attrs = [attr for attr in dir(vectorstore) if not callable(getattr(vectorstore, attr)) and not attr.startswith("__")]
             logger.error(f"Could not access the underlying ChromaDB client from the vectorstore object. Available attributes: {available_attrs}")
             raise HTTPException(status_code=503, detail="Internal configuration error: Could not obtain ChromaDB client from vectorstore.")
        # --- End Client Access ---
        ...
        logger.info(f"Processing results: {len(ids) if ids else 0} IDs, {len(contents) if contents else 0} contents, {len(metadatas) if metadatas else 0} metadatas.")

        if ids is None or contents is None or metadatas is None or not (len(ids) == len(contents) == len(metadatas)):
            logger.error(f"Mismatch or None in lists returned by ChromaDB get: ids({len(ids) if ids else 'None'}), contents({len(contents) if contents else 'None'}), metadatas({len(metadatas) if metadatas else 'None'})")
        ...
        logger.info(f"Successfully processed {len(doc_chunks)} document chunks for response.")
